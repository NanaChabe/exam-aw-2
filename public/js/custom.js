...
$.get('/star', function(data){
  $("#newElement").remove();
   $( data ).each(function( index ) {
      ...
      $('<div id ="newElement" class="col text-center resumen mb-5 p-3"><img src="'+data[index]["picture"]["medium"]+'"class="img-fluid rounded-circle" /><br><h3 class="mt-3">'+data[index]["name"]["first"] + data[index]["name"]["last"]+ '</h3><p>'+ data[index]["email"]+'</p><br><a type="button" href="#element'+index+'" class="btn btn-light mas" id="'+index+'" role="button">Ver detalles</a></div>').prependTo($('#div1'));
    });
  });*/
...
$( "a.mas" ).on("click",function(){
  ...
  //funtions related to nodejs routes for use
  $.get( '/show',parameters, function(data) {
...
$( document ).on("click", ".closer" ,function(event) {
    ...
    //funtions related to nodejs routes for use
    $.get( '/close',parameters, function(data) {
